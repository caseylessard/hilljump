...
export function scoreETFsWithPrefs(
  ...
  const drip12w = data.map((d, i) => {
    const lp = live?.[d.ticker];
    return safe(lp?.drip13wPercent, approxPeriodFromAnnual(returns1y[i], 84));
  });
  ...
  const scored = data.map((d, i) => {
    ...
    // Momentum (DRIP-based): 12w (primary), then 4w, small 26w, 52w
    ...
    const momentum12w = drip12wNorm;
    const momentum52w = drip52wNorm;
    const dripWeightedNorm = clamp(0.5 * drip12wNorm + 0.3 * drip4wNorm + 0.1 * drip26wNorm + 0.1 * drip52wNorm, 0, 1);
    ...
    const scoreCore =
      ...
      to01(core.wMomentum13w)     * momentum12w +
