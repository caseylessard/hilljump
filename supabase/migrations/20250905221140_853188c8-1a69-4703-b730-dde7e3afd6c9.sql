...
create policy read_equity_alerts on public.equity_alerts for select using (true);
create policy read_crypto_alerts on public.crypto_alerts for select using (true);

-- service role inserts (set via Edge Function)
create policy service_insert_equity_alerts
  on public.equity_alerts for insert
  to service_role
  with check (true);

create policy service_insert_crypto_alerts
